...
#define BANKSW (*( volatile __xdata unsigned char *) 0x300 )
...
void vga_plot( unsigned short x, unsigned short y, unsigned char color )
{
    ...
    if ( x < 320 && y < 400 )
    {
        if ( x < 256 )
        {
            ...
            /* mem_addr.addr = ( char *)((( x >> 1 ) + ( y << 8 )) | 0x8000 ) */
            mem_addr.addr_byte[1] = ( unsigned char )x;    /* low byte */
            mem_addr.addr_byte[0] = (( unsigned char ) y ) | 0x80;  /* high byte */
        }
        else
        {
            BANKSW = 4;
            /* mem_addr.addr = ( char *)(( x >> 1 ) + ( y << 6 ) + 36608 ) */
            mem_addr.addr_byte[1] = ( unsigned char )x + ((( unsigned char ) y ) << 6 );  /* low byte */
            mem_addr.addr_byte[0] = ( unsigned char )( y >> 2 ) + 0x90; /* high byte */
        }
        ...
        {
            /* right pixel */
            *mem_addr.addr = ( *mem_addr.addr & 0x0F ) | ( color << 4 );
        }
        else
        {
            /* left pixel */
            *mem_addr.addr = ( *mem_addr.addr & 0xF0 ) | ( color & 0x0F );
        }
