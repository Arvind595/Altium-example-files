/*************************************************************************
**
**  VERSION CONTROL:	@(#)tcpip_global.h	1.1	03/04/15
**
...
struct TCP_SESSION
{
    ...
    Uint16 CALLBACKMEMSPEC(*application) (char resend, TCP_SESSION * session,
...
// last element should have port 0
typedef struct
{
...
typedef struct
{
    ...
    Uint16 CALLBACKMEMSPEC(*application) (Uint16 port, Uint16 datalength);
}
UDP_CLIENTSERVER;
...
typedef struct TCPIP_SETTINGS
{
    ...
    void CALLBACKMEMSPEC(*dns_portip) (Uint16 port, char *ip);	// callback function for incoming DNS ip answers
#endif
#ifdef OPTION_DHCP
    ...
    Uint8 CALLBACKMEMSPEC(*telnet_server_usercheck) (char *user, char *password,
                                                     Uint8 CALLBACKMEMSPEC(**commandprocessor)(char *cmd, char *buf, char ROMMEMSPEC *user, Uint8 userid),
						     Uint8 *userid);
#endif
};
...
extern TCP_SESSION *tcp_create(Uint16 sourceport, Uint32 destip, Uint16 destport,
      			       Uint16 CALLBACKMEMSPEC(*application)
                                   (char resend, TCP_SESSION * session,
                                    Uint16 datalength, Uint8 * tcp_flags));
