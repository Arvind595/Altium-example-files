/*************************************************************************
**
**  VERSION CONTROL:    @(#)http_server.c   1.1 03/04/15
**
...
static ROMMEMSPEC char http_ssitag[] = "<!--#exec var=\"";
 ...
 */
Uint16 CALLBACKMEMSPEC http_server(char resend, TCP_SESSION * session, Uint16 datalength, Uint8 * tcp_flags)
{
...
#endif
{
    ...
    {
	// doublecheck if // not in a parameter
	if (!strchr(url, '?') || (q < strchr(url, '?')))
	{
	    url = q + 2;
...
#endif
{
    ...
    if ((mimetype == MIMETYPE_CGIGET) || (mimetype == MIMETYPE_CGISET))
    {
        ...
        var->cgifunc((unsigned char) ((mimetype == MIMETYPE_CGIGET) ? CGIMODE_GET : CGIMODE_SETRESULT),
                         var, NULL);
    }
...
#ifdef HTTP_CGI_SSIGET
    http_bufwrite_rom_ssi(page->pagedata, (Uint16) (page->len ? page->len : strlen_rom((ROMMEMSPEC char*) page->pagedata)));
#else
    http_bufwrite_rom(page->pagedata, (Uint16) (page->len ? page->len : strlen_rom((ROMMEMSPEC char*) page->pagedata)));
#endif
    }
    else
#endif // HTTP_PAGES
