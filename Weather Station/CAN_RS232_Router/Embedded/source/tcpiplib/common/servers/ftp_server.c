/*************************************************************************
**
**  VERSION CONTROL:    @(#)ftp_server.c    1.2 03/04/17
**
...
static Uint16 CALLBACKMEMSPEC ftpdata_server(char resend, TCP_SESSION * session, Uint16 datalength, Uint8 * tcp_flags);
 ...
 */
Uint16 CALLBACKMEMSPEC ftp_server(char resend, TCP_SESSION * session, Uint16 datalength, Uint8 * tcp_flags)
{
        ...
        *p = '\0';
        ...
        while (*q)
        {
            ...
            *p = '\0';
            ...
            if ((p != NULL) || CMD_WITH_PATH_ONLY(msg))
            {
            if (!*q || !strcmp_rom(q, "."))
            {
