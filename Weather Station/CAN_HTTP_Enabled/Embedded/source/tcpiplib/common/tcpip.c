/*************************************************************************
**
**  VERSION CONTROL:    @(#)tcpip.c 1.1 03/04/15
**
...
static Uint16 udp_process(Uint16 ip_datalength)
{
    ...
    for (clientserver = tcpip_settings.udpclientservers; clientserver->application; ++clientserver)
    {
    if (clientserver->port == port)
    {
        return clientserver->application(port, ip_datalength);
    }
    }
...
static Uint16 tcp_process(Uint16 ip_datalength)
{
        ...
        (session->remoteport == tcp_sourceport) && (session->remoteip == IP->sourceip))
    {
        ...
        received_emptyack = (((session->state == TCP_STATE_SYNACK) || (session->state == TCP_STATE_CONNECTED)) && (TCP->flags == TCP_FLAG_ACK)
                     && !tcp_datalength);
        tcp_datalength =
            session->application(0, session, tcp_datalength, &(TCP->flags));
        ...
        if (received_emptyack && (TCP->flags == TCP_FLAG_ACK) && !tcp_datalength)
        {
    ...
    for (server = tcpip_settings.tcpservers; server->application; ++server)
    {
        ...
        session->application = server->application;
